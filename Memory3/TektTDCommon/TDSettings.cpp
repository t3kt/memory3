...
namespace tekt {
  ...
  void ParamGroup::create(TD::OP_ParameterManager* parManager) {
  ...
  void ParamGroup::load(const TD::OP_Inputs& inputs) {
    for (auto& par : _params) {
      par->load(inputs);
    }
  }
  ...
  void Settings::create(TD::OP_ParameterManager* parManager) {
    for (auto& group : _groups) {
      group->create(parManager);
    }
  }

  void Settings::load(const TD::OP_Inputs& inputs) {
    for (auto& group : _groups) {
      group->load(inputs);
    }
  }
